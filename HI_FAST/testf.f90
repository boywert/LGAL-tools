  ...
  do i=1,2
     do j=1,2
        do k=1,2
           ...
           B2(1,index*N+1:index*N+N) = (A1(1,1:N)*A2(1,1:N) + A1(2,1:N)*A2(2,1:N) + A1(3,1:N)*A2(3,1:N))/B1(1,index*N+1:index*N+N)
           !B2(2,index*N+1:index*N+N) = (A2(1,:)*cos(B1(2,index*N+1:index*N+N))*cos(B1(3,index*N+1:index*N+N)) + A1(2,index*N+1:index*N+N)*sin(B1(2,index*N+1:index*N+N))*cos(B1(3,index*N+1:index*N+N)) - A1(3,:)*sin(B1(3,index*N+1:index*N+N)))/sqrt(A1(1,:)*A1(1,:) + A1(2,:)*A1(2,:) + A1(3,:)*A1(3,:))
