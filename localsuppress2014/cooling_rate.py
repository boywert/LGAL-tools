...
def plot_z(z,models,ax,pos,label=0,bottom=0,top=0):    
	...
	mass = gal[index]['HotGas']# (gal[index]["BulgeMass"]+gal[index]["DiskMass"])
        sum_baryons[index] = numpy.histogram(numpy.log10(mass*1.e10/hubble_h),range=rangen,bins=bins,weights=(numpy.float64(1)*(gal[index]["CoolingRadius"])))
        sum_baryons_sq[index] = numpy.histogram(numpy.log10(mass*1.e10/hubble_h),range=rangen,bins=bins,weights=(numpy.float64(1)*(gal[index]["CoolingRadius"]))**2)
