...
def loadfilter(structfile):
    ...
    filter['MetalDiskMass'] = True
    filter['MetalBulgeMass'] = True
...
def plot_smf_z8(ax):
    ...
    for i in range(len(model_names)):
        ...
        (massftn_x[index],massftn_y[index]) = metallicity_fn(gal[index])

    
    for i in range(len(model_names)):
        index = model_names[i]
        ax.plot(massftn_x[index],massftn_y[index],color=model_plot_colors[i],linestyle=model_plot_patterns[i],label=model_labels[i])
...
def main():
    ...
    plot_smf_z8(ax2)
    #plot_smf_z8(ax3)
    fig.savefig("metal.pdf",bbox_inches='tight',pad_inches=0.1)
    plt.close(fig)
