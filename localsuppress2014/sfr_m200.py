...
def plot_z(z,models,prefix):    
    ...
    for i in range(len(models.model_names)):
        ...
        sum_ASFR[index] = numpy.histogram(numpy.log10(gal[index]["Mvir"]*1.e10),range=rangen,bins=bins,weights=gal[index]["CumulativeSFR"].astype(numpy.float64)*hubble_h/(gal[index]["Mvir"]*.e10))
